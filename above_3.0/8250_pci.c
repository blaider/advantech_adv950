		  ...
		  struct uart_port *port, int idx)
{
	...
	|| dev->device == PCIE_DEVICE_ID_ADVANTECH_0016)
	{
		...
		port->unused[0] = (config485 & (0x01 << ((idx % 4)*2))) ? ACR_DTR_RS232 : activeType;
		port->unused[1] |= readw((void *)(remap + XR_17V35X_ID));

		if( (port->unused[1] & 0x88) == 0x88)
		{
			if(config485 & (0x01 << ((idx % 4)*2)))
				port->unused[0] =  ACR_DTR_RS232;
			else if(config485 & (0x01 << ((idx % 4)*2 + 1)))
				port->unused[0] =  SERIAL_MODE_RS485ORRS422S;
			else 
				port->unused[0] =  SERIAL_MODE_RS422M;
		}
